...
<body class="bodyDesc">
    ...
    $id = $_GET["id"];
    $bestSelling = $bestSellers[$id];
    $nome = $bestSelling["nome"];
    $preco = $bestSelling["preco"];
    $imagem = $bestSelling["imagem"];
    $parc_cj = $bestSelling["parcelamento_max_cj"];
    $parc_sj = $bestSelling["parcelamento_max_sj"];
    $descricao = $bestSelling["descricao"];
    $juros_mes = $bestSelling["juros"];
            ...
            <div class="container-modal">
                <table>
                    ...
                    <tbody>
                        ...
                        while ($parcelas <= $bestSelling["parcelamento_max_cj"]) {
                            $prestacao = 0;
                            $vlrtt = 0; // Valor total a ser pago
                            if ($parcelas <= $bestSelling["parcelamento_max_sj"]) {
                                // Parcelamento sem juros
                                $prestacao = $bestSelling["preco"] / $parcelas;
                                $vlrtt = $bestSelling["preco"]; // Valor total sem juros é o valor do produto
                                echo "<tr>
                                        <td>$parcelas x de</td>
                                        <td>R$" . number_format($prestacao, 2, ',', '') . " sem juros</td>
                                        <td>R$" . number_format($vlrtt, 2, ',', '') . "</td>
                                    </tr>";
                            } else if ($parcelas > $bestSelling["parcelamento_max_sj"] && $parcelas <= $bestSelling["parcelamento_max_cj"]) {
                                // Parcelamento com juros simples a partir da quarta parcela
                                $porc_juros = $bestSelling['juros'] / 100;
                                // Juros aplicados somente a partir da quarta parcela
                                $juros = $bestSelling['preco'] * $porc_juros * ($parcelas - $bestSelling["parcelamento_max_sj"]);
                                $vlrtt = $bestSelling['preco'] + $juros;
                                $prestacao = $vlrtt / $parcelas;
                                echo "<tr>
                                                <td>$parcelas x de</td>
                                                <td>R$" . number_format($prestacao, 2, ',', '') . " com juros (" . $bestSelling['juros'] . "% ao mês)</td>
                                                <td>R$" . number_format($vlrtt, 2, ',', '') . "</td>
                                            </tr>";
                            }
                            $parcelas++;
